...
/** React Bits — CardSwap (JS + CSS → TS), adapted for Jarvis suggested tasks. */
export default function CardSwap({
  ...
  const rendered = childArr.map((child, i) => {
    ...
    return cloneElement(element, {
      ...
      // React 19: ref is a prop; cast keeps forwardRef cards typed.
      ...({ ref: refs[i] } as object),
    });
  });
  ...
  return (
    <div
      ref={container}
      className={["card-swap-container", className].filter(Boolean).join(" ")}
      style={{ width, height }}
    >
      {rendered}
    </div>
