...
- **Cloud vision is twice-gated:** per-customer consent defaults to **deny** (NDA drawings never leave), and authorised customers share a global cap of **5 documents per 10:00→10:00 cycle** across automated ingest and manual upload. A sixth asks for an override worth one document; an override never grants consent. Known drawings answer from their saved card without a call.
