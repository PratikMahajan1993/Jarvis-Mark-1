...
- **One Hermes chat per quote.** A quote start opens a new Hermes conversation. It does not resume `jarvis-default`. Later turns of that same quote stay on that new conversation. Mail, calendar, and shop-sheet turns do not join it. If the first Hermes call is slow, Jarvis does not start a second call while the first is still running.
...
### 2026-09-24 — One Hermes chat per quote

Owner: fix Hermes session behaviour and wipe the current Hermes conversation store. A quote start must open a new Hermes chat, not resume `jarvis-default`. Follow-up answers in that quote stay on that chat. A slow first Hermes call must not start a second call. The shop database stays. The Hermes transcript store was wiped the same night.
