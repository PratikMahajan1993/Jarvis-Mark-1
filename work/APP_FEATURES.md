...
- **Machine Hour Rate (interview Q5):** each machine type has a fixed **minimum** MHR. Quotes must not go below it; the quoted rate may be higher. The **demo table in the local DB stays the production source** (locked 2026-09-22) and the owner fills it with real shop rates while the Master data UI is deferred — so a rate is quotable only once the owner has attested it and it no longer holds the shipped seed number. Missing machine, missing rate, or an unlisted machine blocks the send instead of skipping the check. Brain does not invent the floor.
...
- **Before send (interview Q7):** the whole quotation is checked; every price line must be present. If the raw-material supplier quote is not in yet, the owner may **estimate** from historical transactions and the market trend and put that figure in — it must be marked as an estimate and trace to those sources, not a free guess. **Delivery (locked 2026-09-22):** Jarvis never guesses or calculates a delivery date, and insists on having one — it asks while drafting and again on the Authorize card, which carries the field inline. A missing date **warns** at draft (pricing review must not be blocked) and **blocks** the final Authorize to send. Q7's "delivery does not hold the quote" governs the draft; this governs the send. Customer name and the rest of the party data come from Master data later.
...
- **Cloud vision (locked 2026-09-22):** default **deny** per customer to protect NDA drawings. **Only the owner spends a unit** — automated mail ingest does the free local text extract, flags the drawing `needs_vision`, and waits on the bench; the cloud call happens when the owner opens that RFQ. Authorised customers share a cap of **5 documents per cycle running 10:00 → 10:00**; a sixth pauses and asks for an override worth one document, and a pack over four sheets asks before spending. Known drawings answer from the saved card for free.
- **Raw-material freshness (locked 2026-09-22):** a raw-material price basis older than **30 days** blocks the send, with no override — supplier quote, invoice, or marked estimate alike.
- **Sign-off (locked 2026-09-22):** the owner is the only commercial signer. No approval tiers, no delegation. A queued quote Authorize must never expire quietly.
- **Tool wear (locked 2026-09-22):** `toolwatch` v0 learns from the manual shop log only — one spoken or typed line per insert change. No machine-network or sensor integration in this phase.
- **Memory (locked 2026-09-22):** 100 % local — SQLite plus LanceDB, no cloud memory service.
...
### 2026-09-22 — Seven executive decisions locked

Owner reviewed the architecture manifest and locked: cloud vision default-deny per customer with a 5-drawing cap per 10:00→10:00 cycle across all sources and a manual override; the demo rate table stays and gets real owner-attested rates while the Master data UI waits; sole commercial sign-off, no approval tiers; `toolwatch` v0 on the manual shop log with FOCAS/MTConnect deferred; a hard 30-day block on raw-material price basis; Honcho stripped for 100 % local memory; and no guessed delivery dates — the field stays empty and blocks Authorize until the owner fills it. Detail and mechanisms: [`OPUS_ARCHITECTURE_MANIFEST.md`](OPUS_ARCHITECTURE_MANIFEST.md) §8.

This reverses one earlier answer: interview Q7 said delivery time does not hold the quote. The new rule blocks at Authorize, not at build, on the reading that Q7 meant Jarvis must not wait on a *supplier* lead time.
